 ...
 *
 *
 *  Source: reference from Mathworks: https://microem.ru/files/2012/08/GPS.G1-X-00006.pdf
 * http://www.mathworks.com/help/aeroblks/ecefpositiontolla.html
 */
object Ecef2LlaConverter {
    ...
    private const val MIN_MAGNITUDE_METERS = 1.0e-22
    private const val MAX_ITERATIONS = 15.0
    private const val RESIDUAL_TOLERANCE = 1.0e-6
    private val SEMI_MINOR_AXIS_METERS = Math.sqrt(Math.pow(EARTH_SEMI_MAJOR_AXIS_METERS, 2.0) * (1 - Math.pow(ECCENTRICITY, 2.0)))
    private val SECOND_ECCENTRICITY = Math.sqrt((Math.pow(EARTH_SEMI_MAJOR_AXIS_METERS, 2.0) - Math.pow(SEMI_MINOR_AXIS_METERS, 2.0))
            / Math.pow(SEMI_MINOR_AXIS_METERS, 2.0))
    private const val ECEF_NEAR_POLE_THRESHOLD_METERS = 1.0
     ...
     *
     *
     * Output is GeodeticLlaValues class containing geodetic latitude (radians), geodetic longitude
     * (radians), height above WGS84 ellipsoid (m)}
     */
    fun convertECEFToLLACloseForm(ecefXMeters: Double, ecefYMeters: Double,
                                  ecefZMeters: Double): GeodeticLlaValues {

        // Auxiliary parameters
        val pMeters = Math.sqrt(Math.pow(ecefXMeters, 2.0) + Math.pow(ecefYMeters, 2.0))
        val thetaRadians = Math.atan2(EARTH_SEMI_MAJOR_AXIS_METERS * ecefZMeters, SEMI_MINOR_AXIS_METERS * pMeters)
        var lngRadians = Math.atan2(ecefYMeters, ecefXMeters)
        // limit longitude to range of 0 to 2Pi
        lngRadians = lngRadians % (2 * Math.PI)
        val sinTheta = Math.sin(thetaRadians)
        val cosTheta = Math.cos(thetaRadians)
        val tempY = (ecefZMeters
                + Math.pow(SECOND_ECCENTRICITY, 2.0) * SEMI_MINOR_AXIS_METERS * Math.pow(sinTheta, 3.0))
        val tempX = (pMeters
                - Math.pow(ECCENTRICITY, 2.0) * EARTH_SEMI_MAJOR_AXIS_METERS * Math.pow(cosTheta, 3.0))
        val latRadians = Math.atan2(tempY, tempX)
        // Radius of curvature in the vertical prime
        val curvatureRadius = (EARTH_SEMI_MAJOR_AXIS_METERS
                / Math.sqrt(1 - Math.pow(ECCENTRICITY, 2.0) * Math.pow(Math.sin(latRadians), 2.0)))
        var altMeters = pMeters / Math.cos(latRadians) - curvatureRadius

        // Correct for numerical instability in altitude near poles
        val polesCheck = (Math.abs(ecefXMeters) < ECEF_NEAR_POLE_THRESHOLD_METERS
                && Math.abs(ecefYMeters) < ECEF_NEAR_POLE_THRESHOLD_METERS)
        if (polesCheck) {
            altMeters = Math.abs(ecefZMeters) - SEMI_MINOR_AXIS_METERS
        }
        return GeodeticLlaValues(latRadians, lngRadians, altMeters)
    }
     ...
     *
     *
     * Outputs is GeodeticLlaValues containing geodetic latitude (radians), geodetic longitude
     * (radians), height above WGS84 ellipsoid (m)}
     */
    fun convertECEFToLLAByIterations(ecefXMeters: Double,
                                     ecefYMeters: Double, ecefZMeters: Double): GeodeticLlaValues {
        val xyLengthMeters = Math.sqrt(Math.pow(ecefXMeters, 2.0) + Math.pow(ecefYMeters, 2.0))
        val xyzLengthMeters = Math.sqrt(Math.pow(xyLengthMeters, 2.0) + Math.pow(ecefZMeters, 2.0))
        val lngRad: Double
        lngRad = if (xyLengthMeters > MIN_MAGNITUDE_METERS) {
            Math.atan2(ecefYMeters, ecefXMeters)
        } else {
            0.0
        }
        var sinPhi: Double
        sinPhi = if (xyzLengthMeters > MIN_MAGNITUDE_METERS) {
            ecefZMeters / xyzLengthMeters
        } else {
            0.0
        }
        // initial latitude (iterate next to improve accuracy)
        var latRad = Math.asin(sinPhi)
        var altMeters: Double
        if (xyzLengthMeters > MIN_MAGNITUDE_METERS) {
            var ni: Double
            var pResidual: Double
            var ecefZMetersResidual: Double
            // initial height (iterate next to improve accuracy)
            altMeters = xyzLengthMeters - EARTH_SEMI_MAJOR_AXIS_METERS * 1 - sinPhi * sinPhi / INVERSE_FLATTENING
            var i = 1
        ...
        return GeodeticLlaValues(latRad, lngRad, altMeters)
    }
     ...
     */
    class GeodeticLlaValues(val latitudeRadians: Double,
                            val longitudeRadians: Double, val altitudeMeters: Double)
}